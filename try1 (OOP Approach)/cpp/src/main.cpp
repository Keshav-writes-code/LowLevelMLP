...
int main() {
  ...
  const int hidOutLayerCount = 7;
  const int outputLayerSize = 10;
  int *hidOutLayerSizes = genRandomInts(hidOutLayerCount, 20, 200);
  NeuralNet::MLP *MLP = new NeuralNet::MLP(
      inputLayerSize, hidOutLayerCount, hidOutLayerSizes, outputLayerSize, 0.3);
